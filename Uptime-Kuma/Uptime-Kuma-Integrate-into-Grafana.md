...
- Uptime‑Kuma running and reachable from the Prometheus server (example: `http://10.210.2.112:30001`).
...
curl -sS -i http://10.210.2.112:30001/metrics | head -n 20
```
...
2. Find the `uptime` job. You should see your `10.210.2.112:30001` target listed and `UP`.
...
- Set it to `10.210.2.112:30001` (or whatever label Prometheus shows under `instance`) to populate panels.
